...
- 修复「内置用户」权限可越权升级为管理员：只持有 `users` 权限的账号此前可以通过 `POST/PATCH /api/users` 把自己或他人设为 `admin`（`admin` 会绕过全部权限校验），也可以重置/删除管理员账号；现在授予 `admin` 角色与操作管理员账号仅限管理员，`users` 权限对普通用户的管理（改名、停用、重置密码、分配自己已有的权限）保持不变

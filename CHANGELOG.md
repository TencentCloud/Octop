...
- 版本升级检查支持 PyPI 探测重试与镜像 JSON 回退（pypi.org → 腾讯云 → 阿里云），官方源 3s 短超时快速失败、镜像 8s，并在状态接口透出 `error_code` 与 `source` 字段，供界面本地化错误文案与提示版本来源
...
- 版本升级界面偶现 "could not reach PyPI"：失败的检查结果不再缓存 1 小时（前后端改为 5 分钟短缓存）；失败态下升级页每分钟自动重探、缓存过期判断与轮询共用同一套 TTL，网络恢复后红字自动消失；界面错误文案支持中英文本地化，来自镜像时显示来源提示，`octop update` 失败提示补充排查建议

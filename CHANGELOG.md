...
### 修复
- 远程 Android 的 adb 输出改按 UTF-8 解码：此前跟随系统 ANSI 代码页，中文 Windows（cp936）上设备型号等设备侧文本变成乱码（「小米」显示为「潏忕背」），遇到该代码页无法表示的字节时整段输出直接丢失，`list_devices()` 还会抛 `AttributeError`（#1124，与 #1118 同族）

...
- 聊天**运行轨迹**改为宽抽屉打开（与工作台一致），采用 DeepSeek 风格时间线交互、检查器与工具栏

### 新增

- 聊天运行轨迹宽抽屉：时间线泳道、检索与折叠、会话指标与导出；点击事件可查看完整载荷
- 运行轨迹从 harness 实况汇聚回合上下文（记忆 / 技能等），并为工具突发合成 ASSISTANT 父节点
- 知识库支持更多文本与表格格式（csv / tsv / html / json / yaml / xls / xlsx / xlsm 等）
- 知识库可选 OCR：本地 RapidOCR（ONNX）或在线视觉模型；支持 png / jpg / webp，扫描 PDF 无文字时自动回退
- 知识库上传显示逐文件进度
- 聊天消息列表左侧回合时间轴：按用户回合显示刻度，悬停预览、点击跳转、滚动联动；宽屏且至少两轮对话时显示

...
### 修复
- 飞牛 fnOS 包安装 / 卸载不再静默杀掉占用 8088、8089 的第三方进程（例如把这两个端口映射出去的容器 `docker-proxy`）：`free_octop_ports` 现在只清理命令行指向本安装目录或 `octop.cli.main` 的本应用残留进程，端口被他人占用时只在安装日志里提示改用其它端口；本地版「设置」窗口保存时也不再把手改过的 `OCTOP_PORT` 静默写回 8089，`octop-login.txt` 里的访问地址同步按实际端口生成 (#985)

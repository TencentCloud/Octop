...
- `tests/unit` 不再依赖系统 ANSI 代码页：读迁移 SQL 的 8 个测试文件补上 `encoding="utf-8"`，与 `octop.infra.db.migrate` 自己的读取方式一致；中文 Windows（cp936）上跑整套 `tests/unit` 此前固定红 19 条，现在全绿（#1062）

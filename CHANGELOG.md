...
- 远程手机：主机缺少 Docker 时自动安装——纯测速选择安装源（官方 download.docker.com 与境内镜像竞速，官方最快则不设 DOWNLOAD_URL），执行官方 get.docker.com 脚本；安装后按腾讯云镜像加速可达性决定是否写入 `registry-mirrors`（不检测地区，连不上则跳过）。自动安装失败时回退为提示用户手动安装。

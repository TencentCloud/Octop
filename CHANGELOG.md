...
- CLI `provider` / `channel` / `cron` 的离线写命令完成后会探测运行中的 `octop run`（读 config.json 的监听地址探 `/api/health`），命中则在 stderr 输出本地化提示「需重启后生效」；`cron create` 原有的无条件英文提示（打到 stdout、会污染 `--json` 输出）一并替换（#952）
- 对话错误透传真实根因：`format_stream_error` 在把原始异常替换成本地化通用文案时追加原始原因（凭证脱敏 + 截断），HTTP / WS / CLI / IM 四个对话通道同时生效——此前 provider/模型解析失败只显示「模型调用多次重试后仍失败」，无从定位真实原因（#952，与 #852 同主题）

...
- 定时任务空状态提示词按专家区分：workspace → 目录模板 → 名称默认，不再共用一套通用文案（#651）
- fnOS 包不再宣称 `platform=all`：native / Docker 包实际只含 x86_64 构建产物，改为如实声明 `platform=x86`，并在 ARM64 设备上启动时给出明确拒绝提示（#829）

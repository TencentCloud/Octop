...
- MCP 日志降噪：服务端以 405 拒绝 GET SSE（符合 Streamable HTTP 规范，如 GitHub Copilot MCP）时，不再每 ~10 秒刷出误导性的「GET stream disconnected, reconnecting」重连日志；`OCTOP_LOG_LEVEL=debug` 下仍保留完整轨迹 (#487)

...
- 知识库文档预览打磨：Markdown 大纲与文内查找、上下篇切换、全屏；PDF 窗口化渲染、书签/缩放/适合宽高、分阶段加载与下载进度；Excel 嵌入图改为图集；Office 大文件下载进度
- 聊天知识库引用来源在对话右侧 dock 中预览（与「编辑了文件」同一面板）；悬停显示文件与知识库信息，并可跳转知识库定位打开（#634）

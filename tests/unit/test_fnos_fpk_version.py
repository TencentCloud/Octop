...
The .fpk version comes from ``pyproject.toml`` through a sed pattern that only
accepts digits and dots. A prerelease such as ``1.0.2b1`` (the version on
``develop`` and the tag ``v1.0.2b1``) did not match, and because sed passes a
non-matching line through unchanged, ``$VER`` became the whole

...
async def test_set_role_visible_to_other_worker(tmp_path: Path) -> None:
...
async def test_disable_revokes_other_worker(tmp_path: Path) -> None:
...
async def test_remove_revokes_other_worker(tmp_path: Path) -> None:
...
async def test_authenticate_returns_the_stored_role(tmp_path: Path) -> None:

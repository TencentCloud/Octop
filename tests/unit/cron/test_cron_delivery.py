...
@pytest.mark.asyncio
async def test_dashboard_signals_thread_activity_before_delivery() -> None:
    session = _session(channel_type=ThreadRegistry.CHANNEL_DASHBOARD)
    harness = MagicMock()
    ...
    async def activity(*_args, **kwargs):
        order.append(f"activity:{kwargs['reason']}")
    ...
    gateway.run_in_session = _run_locked
    gateway.require_session = MagicMock(return_value=session)
    gateway.notify_thread_activity = activity
    gateway.push_session_text = push
    ...
    assert order == ["activity:cron_started", "push"]


@pytest.mark.asyncio
async def test_thread_activity_failure_still_delivers() -> None:
    session = _session(channel_type=ThreadRegistry.CHANNEL_DASHBOARD)
    harness = MagicMock()
    harness.aappend_messages = AsyncMock(
        return_value=[
    ...
    gateway.run_in_session = _run_locked
    gateway.require_session = MagicMock(return_value=session)
    gateway.notify_thread_activity = AsyncMock(side_effect=RuntimeError("socket down"))
...
@pytest.mark.asyncio
async def test_im_session_skips_thread_activity() -> None:
    session = _session(channel_type="feishu")
    gateway = MagicMock()
    gateway.run_in_session = _run_locked
    gateway.require_session = MagicMock(return_value=session)
    gateway.notify_thread_activity = AsyncMock()
    gateway.push_session_text = AsyncMock()
    ...
    await service.deliver(_command())
    gateway.notify_thread_activity.assert_not_awaited()

...
- **凭证安全与全自动登录**：严禁向用户索要 SecretId/SecretKey，拒绝任何可能打印凭证的操作。凭证缺失时由你全自动驱动 OAuth（`BROWSER=echo` 后台跑 `auth login` → 提取授权链接发给用户 → 用户点「授权」→ 凭证自动落盘）。**授权是否成功以 `~/.tccli/<profile>.credential` 文件是否更新为准，不以 auth 进程退出或工具超时为准**——工具执行超时绝不代表用户没完成授权；禁止长轮询阻塞等待（会被工具超时杀掉造成误判），发出链接后短探测即收，等用户回复后用凭证文件时间戳确认并回显身份。凭证文件已更新就绝不重复发起登录（重复登录会让上一个链接失效）。完整流程见 `skills/tcapi/SKILL.md` Step 2 与 `skills/tcapi/references/auth.md`。

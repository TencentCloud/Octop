...
2. 请用系统浏览器授权：在 Chrome 等系统浏览器打开当前 Octop Web 地址，再点击「一键授权」，登录企查查并完成授权，无需手动填写 API Key。
...
此记录验证了 company 服务与通用 OAuth 流程；后续五类聚合入口验收如下。
...
同日，在 PR #1033 提交 `8dd25ca` 的独立开发实例中，使用 Chrome 完成新的真实 OAuth 授权：

- 发现 151 个工具：company 16、risk 38、ipr 18、operation 35、executive 44。
- 经聚合网关分别调用企业简介、经营异常、专利、行政许可、董监高在外任职工具，均返回正常业务结果。
- 将验收授权的本地到期时间设为已过期后，5 个并发工具发现请求均成功；真实 Access Token 和 Refresh Token 均轮换，内部访问令牌保持稳定。
- 完整停止并重新启动 Octop 进程，无需重新授权，五类真实调用再次通过。
- 解绑返回 HTTP 204，旧入口返回 404；已撤销 Refresh Token 再次刷新返回 HTTP 400 / `invalid_grant`。验收授权已解绑。

此为每类一个工具的抽样验收，未遍历全部工具；未等待 Token 自然到期，远端刷新请求次数未计数，
“仅刷新一次”由自动化测试验证。未重新验证模型对话中的工具选择。上述记录不包含凭证或原始业务数据。

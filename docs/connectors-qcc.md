...
4. 完成登录授权后，确认 Octop 显示已授权、探测成功，在对话中选择卡片并使用默认执行模式（Craft），输入企业全称及查询需求。Ask / Plan 模式按产品设计禁用 MCP，不能用于验证连接器调用。
...
这是五类资源各一项的抽样验收；没有逐一验证全部上游工具。公网 HTTPS 回调、桌面内嵌弹窗及多进程部署未做真实端到端验收。公网部署仍需按上述回调要求配置并单独验证。

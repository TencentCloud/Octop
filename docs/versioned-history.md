...
本期明确阻止涉及 v2 数据的**在线**恢复入口（HTTP 接口）与运行时换库，防止只恢复主库造成错配。
停机的 `octop backup restore`（离线 CLI，需先停止服务）会一次性恢复同一备份中的主库、
`history_v2.sqlite` 与标记文件：备份包含聊天时直接用它恢复即可，备份不含聊天时保持现存归档不动。
Windows 上若归档仍被占用（例如服务未停、索引器/杀软临时持有），替换会短暂重试后报
“the versioned history archive is in use; stop the server and retry the restore”，此时现存归档保持不变。
跨机器或换路径恢复仍需按下面的手工步骤处理：新归档记录主库的绝对路径身份，
后续版本才提供显式身份重绑定工具。手工恢复时：停止服务、另存当前整个目录、
在原路径恢复同一备份中的主库、新归档及标记、agent checkpoint/工作区、配置，
然后用兼容版本启动检查。
